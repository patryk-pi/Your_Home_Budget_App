...
const GoalSetForm = ({ description, type }) => {
    ...
    useEffect(() => {
        setMonthAndYear(
            dayjs(`${currentYear}-${currentMonth + 1}`).format("MM/YYYY")
        );
    }, [currentMonth, currentYear]);

    useEffect(() => {
        setFilteredRecords(
            goals.filter((goalRecord) => filterGoalsByMonth(goalRecord))
        );
    ...
    const handleSubmit = useCallback(
        ...
        [monthAndYear, category, type, goal, setCategoryAndGoal, setIsDisabled]
    );
    ...
    useEffect(() => {
        if (
            categoryAndGoal.monthAndYear &&
            categoryAndGoal.category &&
            categoryAndGoal.goal !== null
        ) {
